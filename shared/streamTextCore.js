import { resolveMarkdownCommandLines, removeSkippedRegions } from './markdownCommandsCore.js';
...
export function normalizeFencedCodeBlocksForSpeech(text) {
...
export function splitStreamParagraphChunks(text, startIndex) {
  const input = stripMarkdown(text.slice(Math.max(0, startIndex)));
